...
50. **Razorpay Standard Web Checkout**: Frontend `BillHubTab.tsx`, `PatientMobileDashboard.tsx`, and `CompounderDashboard.tsx` MUST provide prominent **`Razorpay (Online Gateway)`** options that dynamically load `checkout.js`, open standard Razorpay modal with server-validated `order_id`, and verify server-side HMAC-SHA256 signature via `razorpay-verify` before marking invoices as paid.

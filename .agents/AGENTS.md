...
51. **TypeScript Strict Type Safety**: `BillingService.clearInvoice` MUST include `'razorpay'` \| `'cashfree'` in `paymentMethod` union types, and patient references MUST evaluate active scope variables (`activePatient`) to ensure `tsc -b --force` Vercel builds pass cleanly.

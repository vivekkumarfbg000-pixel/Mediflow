...
50. **Razorpay Standard Web Checkout**: Frontend `BillHubTab.tsx` and `PatientMobileDashboard.tsx` MUST provide a prominent **`Pay via Razorpay`** button that dynamically loads `checkout.js`, opens standard Razorpay modal with backend `order_id`, and verifies server-side HMAC-SHA256 signature via `razorpay-verify` before marking invoices as paid.
51. **TypeScript Strict Type Safety**: `BillingService.clearInvoice`, `BillingService.recordInvoicePayment`, `BillingService.markInvoicePaid`, and `api.ts` MUST include `'razorpay'` \| `'cashfree'` in `paymentMethod` union types, and patient references MUST evaluate active scope variables (`activePatient`) to ensure `tsc -b --force` Vercel builds pass cleanly.

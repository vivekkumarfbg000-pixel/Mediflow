...
`;
  ...
  useEffect(() => {
    const VALID_ADMIN_TABS = new Set(['saas_health', 'ai_fleet', 'onboarding', 'revenue', 'costs', 'firewall']);
    const handleAdminTabChange = (e: Event) => {
      const customEvent = e as CustomEvent<string>;
      const target = customEvent.detail;
      if (target && VALID_ADMIN_TABS.has(target)) {
  ...
  if (isAdmin) {
    ...
    const handleSwitchTab = (tabId: ActiveTab) => {
      setActiveTab(tabId);
    ...
    return (
      <div 
        className="max-w-7xl mx-auto p-3 sm:p-6 lg:p-8 space-y-4 sm:space-y-5 pb-32"
        style={{ paddingTop: 'env(safe-area-inset-top, 16px)' }}
      >
        
        {/* ── Native Mobile App Header (Mobile Screens Only) ───────────────────────── */}
        <div className="block sm:hidden space-y-2.5 pb-1">
          {/* Top Bar: Identity & Quick Actions */}
          <div className="flex items-center justify-between gap-2 p-3 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl rounded-2xl border border-slate-200/80 dark:border-white/10 shadow-2xs">
            <div className="flex items-center gap-2.5 min-w-0">
              <div className="w-8.5 h-8.5 rounded-xl bg-gradient-to-br from-indigo-500 to-cyan-500 flex items-center justify-center text-white shadow-xs shrink-0">
                <Terminal className="h-4 w-4" />
              </div>
              <div className="min-w-0">
                <h2 className="text-xs font-black text-slate-850 dark:text-white tracking-tight flex items-center gap-1.5 truncate leading-tight">
                  VitalSync Admin
                  <span className="inline-block w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse shrink-0" />
                </h2>
                <span className="text-[8.5px] font-mono font-extrabold text-indigo-600 dark:text-indigo-400 uppercase tracking-wider block truncate">
                  Platform Cockpit
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              <button
                ...
                className="h-7.5 px-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-xl font-black text-[9.5px] flex items-center gap-1 shadow-xs active:scale-95 transition-all cursor-pointer"
          ...
          {/* Quick Auto-Pilot & E2E Simulator Strip */}
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              className={`flex items-center justify-center gap-1.5 h-8 px-2.5 rounded-xl border text-[9.5px] font-extrabold transition-all cursor-pointer active:scale-95 ${
                isAutoPilotEnabled 
                  ? 'bg-emerald-50 dark:bg-emerald-950/60 border-emerald-300/80 dark:border-emerald-800 text-emerald-700 dark:text-emerald-300' 
                  : 'bg-slate-100 dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400'
              }`}
            >
              <span className={`w-1.5 h-1.5 rounded-full ${isAutoPilotEnabled ? 'bg-emerald-500 animate-pulse' : 'bg-slate-400'}`} />
            ...
            <button
              ...
              className="flex items-center justify-center gap-1.5 h-8 px-2.5 rounded-xl border border-indigo-200 dark:border-indigo-800/60 bg-indigo-50 dark:bg-indigo-950/50 text-indigo-700 dark:text-indigo-300 font-extrabold text-[9.5px] transition-all cursor-pointer active:scale-95"
            >
          ...
          {/* Horizontal Swipeable Segmented Tab Controller */}
          <div className="flex items-center gap-1.5 p-1 bg-slate-100/80 dark:bg-slate-900/80 backdrop-blur-md rounded-2xl border border-slate-200/60 dark:border-white/5 overflow-x-auto no-scrollbar">
            {agents.map(agent => {
              ...
              return (
                <button
                  key={agent.id}
                  type="button"
                  onClick={() => handleSwitchTab(agent.id)}
                  className={`flex items-center gap-1.5 py-1.5 px-2.5 rounded-xl text-[10px] font-bold transition-all whitespace-nowrap shrink-0 cursor-pointer active:scale-95 ${
                    isActive 
                ...
                >
                  ...
                  <span>{agent.label.replace(' Auto Agent', '').replace(' Agent', '')}</span>
                </button>
              );
            })}
          </div>
        ...
        {/* ── Desktop Header Control Bar (Desktop Screens) ─────────────────────── */}
        <div className="hidden sm:flex flex-col sm:flex-row justify-between sm:items-center gap-3 border-b border-slate-200/60 dark:border-white/10 pb-4">
        ...
        {/* ── Virtual Operations Team Navigation (Desktop Header) ───────────────── */}
        <div className="hidden sm:flex items-center gap-2 bg-slate-50 dark:bg-slate-900/50 p-2.5 rounded-2xl border border-slate-200/60 dark:border-white/10 mb-5 overflow-x-auto no-scrollbar">
          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1 shrink-0">
            Virtual Team:
          </span>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5 sm:gap-4">
                ...
                <div className="p-5 rounded-3xl border border-slate-200 bg-white space-y-4">
                  ...
                  {syntheticProfiles.length > 0 ? (
                    <>
                      {/* Mobile App Cards View for Synthetic Profiles */}
                      <div className="block md:hidden space-y-2.5">
                        {syntheticProfiles.map(profile => (
                          <div key={profile.id} className="p-3 bg-slate-50/80 border border-slate-200/80 rounded-xl space-y-2 text-left">
                            <div className="flex justify-between items-start">
                              <div>
                                <h5 className="font-extrabold text-xs text-slate-800">{profile.name}</h5>
                                <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[8.5px] font-bold uppercase tracking-wider bg-indigo-50 text-indigo-650 mt-1">
                                  {profile.role}
                                </span>
                              </div>
                            ...
                            <div className="flex justify-between items-center text-[10px] p-2 bg-white rounded-lg border border-slate-200/50">
                              <div>
                                <span className="text-[8.5px] font-bold uppercase text-slate-400 block">Interactions</span>
                                <span className="font-mono font-bold text-slate-700">{profile.associatedActivityMetric.interactionsCount}</span>
                              </div>
                              <div className="text-right">
                                <span className="text-[8.5px] font-bold uppercase text-slate-400 block">Last Active</span>
                                <span className="font-mono text-slate-500">{new Date(profile.associatedActivityMetric.lastActive).toLocaleTimeString()}</span>
                              </div>
                            </div>
                      ...
                      {/* Desktop Table View */}
                      <div className="hidden md:block border border-slate-100 rounded-xl overflow-hidden lg:max-h-[300px] max-h-none lg:overflow-y-auto overflow-x-auto responsive-table-container">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                  ...
                  <div className="lg:col-span-7 p-5 rounded-3xl border border-slate-200 bg-white space-y-3">
                    ...
                    {loadingFirewall ? (
                      ...
                      <>
                        {/* Mobile App Cards View for Blacklisted IPs */}
                        <div className="block md:hidden space-y-2">
                          {blacklistedIps.map(rule => (
                            <div key={rule.ip} className="p-3 bg-slate-50/80 border border-slate-200/80 rounded-xl flex items-center justify-between gap-2 text-left">
                              <div>
                                <span className="font-mono text-xs font-bold text-rose-600">{rule.ip}</span>
                                <p className="text-[10px] text-slate-500 mt-0.5 leading-snug">{rule.reason}</p>
                              </div>
                              <button
                                type="button"
                                onClick={() => handleRemoveIpFromBlacklist(rule.ip)}
                                disabled={removingIp === rule.ip}
                                className="h-8 px-2.5 bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200/60 rounded-lg text-[9.5px] font-extrabold uppercase shrink-0 transition-all"
                              >
                                {removingIp === rule.ip ? <Loader2 className="h-3 w-3 animate-spin" /> : 'Unblock'}
                              </button>
                            </div>
                          ))}
                        </div>
                  ...
                  <div className="lg:col-span-5 p-5 rounded-3xl border border-slate-200 bg-white space-y-3">
                    ...
                    {loadingFirewall ? (
                      ...
                      <>
                        {/* Mobile App Cards View for Rate Limits */}
                        <div className="block md:hidden space-y-2">
                          {rateLimits.map(log => {
                            const ageSec = Math.max(0, Math.floor((Date.now() - new Date(log.window_start).getTime()) / 1000));
                            return (
                              <div key={log.ip} className="p-2.5 bg-slate-50/80 border border-slate-200/80 rounded-xl flex items-center justify-between text-xs">
                                <div>
                                  <span className="font-mono text-slate-800 font-bold text-xs">{log.ip}</span>
                                  <div className="text-[9.5px] text-slate-400 font-mono mt-0.5">{ageSec < 60 ? `${ageSec}s ago` : `${Math.floor(ageSec / 60)}m ago`}</div>
                                </div>
                                <span className={`px-2 py-0.5 rounded-full font-mono font-extrabold text-[10px] ${
                                  log.request_count >= 15 ? 'bg-amber-100 text-amber-700 animate-pulse' : 'bg-slate-200 text-slate-700'
                                }`}>

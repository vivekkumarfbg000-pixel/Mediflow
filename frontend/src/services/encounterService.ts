...
export class EncounterService {
  ...
  static createEncounter(encounterData: Omit<Encounter, 'id' | 'createdAt' | 'status'>): Encounter {
    ...
    if ((newEncounter.diagnosticTests || []).length > 0) {
      ...
      for (const test of newEncounter.diagnosticTests) {
        ...
        dbReqsToInsert.push({
          ...
          encounter_id: encounterId,
          patient_id: newEncounter.patientId,
          loinc_code: test.loincCode || (test as any).testCode || 'LOINC-001',
          test_code: test.loincCode || (test as any).testCode || 'LOINC-001',
          test_name: test.name,
          barcode: barcode,
          status: 'pending',
          pod_id: ctx.podId,
    ...
    const platFee = isPureCounterConsult ? 0 : parseFloat(((labFee * 0.02) + (pharmFee * 0.01)).toFixed(2));

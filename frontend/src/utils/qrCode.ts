// Pure TypeScript QR Code Engine based on Project Nayuki (MIT License)
// 100% compliant with ISO/IEC 18004, Model 2, Versions 1-40.
// Zero dependencies, erasableSyntaxOnly compliant (no namespace, no enum).
...
export class QrMode {
  ...
  public numCharCountBits(ver: int): int {
	...
	export class QrCode {
		...
		public static encodeBinary(data: Readonly<Array<byte>>, ecl: QrEcc): QrCode {
		...
		private static reedSolomonComputeRemainder(data: Readonly<Array<byte>>, divisor: Readonly<Array<byte>>): Array<byte> {
			...
			for (const b of data) {  // Polynomial division
				...
				result.push(0);
				divisor.forEach((coef: byte, i: int) =>
...
(QrCode as any).Ecc = QrEcc;
(QrSegment as any).Mode = QrMode;

/**
 * Generate a 2D boolean module matrix: true for dark, false for light.
 */
export function generateQRMatrix(text: string, ecl: 'L' | 'M' | 'Q' | 'H' = 'M'): { matrix: boolean[][]; size: number } {
  const eccMap = {
    L: QrEcc.LOW,
    M: QrEcc.MEDIUM,
    Q: QrEcc.QUARTILE,
  ...
  const qr = QrCode.encodeText(text, eccMap[ecl] || QrEcc.MEDIUM);
  const size = qr.size;
...
export function generateQRCodeSVG(
  ...
  try {
    const { matrix, size } = generateQRMatrix(text, options?.ecl || 'M');
    const color = options?.color || '#0f172a';
    const bgColor = options?.bgColor || '#ffffff';
    const margin = options?.margin !== undefined ? options.margin : 2;
    const totalModules = size + margin * 2;
    const pixelSize = options?.size || 300;
    ...
    return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${totalModules} ${totalModules}" width="${pixelSize}" height="${pixelSize}" shape-rendering="crispEdges">
  <rect width="${totalModules}" height="${totalModules}" fill="${bgColor}" />
  <path d="${pathData}" fill="${color}" />
</svg>`;
  } catch (err) {
    console.error('[VitalSync QR] Client-side SVG generation error:', err);
    return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" width="300" height="300">
  <rect width="100" height="100" fill="#ffffff" />
  <text x="50" y="50" font-size="8" text-anchor="middle" fill="#0f172a">QR Code Error</text>
</svg>`;
  }
}

/**
 * Generate a safe Data URI string suitable for <img src="..." />
 * Uses standard W3C 'image/svg+xml;charset=utf-8' format and encodeURIComponent
 */

...
export const OPHTHALMIC_TEST_CATALOG: DiagnosticTest[] = [
  { loincCode: '79892-0', name: 'OCT Macular Scan', category: 'Retinal Imaging', normalRange: 'N/A', unit: '╬╝m', price: 1500 },
...
class MediflowApiService {
      ...
      switch (session.currentState) {
        case 'AWAITING_WELCOME':
          if (['1', 'grant access', 'yes', 'approve', 'grant'].includes(cleaned)) {
            ...
            replyMessage = "Bahut bahut dhanyawad! Aapka clinical consent safe tarike se secure ledger mein register ho gaya hai. State: READY_FOR_ENCOUNTER. ≡ƒƒó";
            ...
            replyMessage = "Hum samajh nahi paaye. ≡ƒñû Apne records safe sync karne ke liye please upar click kijiye ya bas *1* reply karke authorize kijiye.";
          }
          break;
        ...
        case 'AWAITING_CONFIRMATION':
          if (['stop consent', 'stop', 'revoke', 'stop_consent'].includes(cleaned)) {
            ...
            replyMessage = "Aapka clinical consent active hai! ≡ƒƒó Batayein main aapki kya help karoon? Reply kijiye:\n- *BOOK*: Doctor appointment book karne ke liye\n- *REPORT*: Apni lab reports dekhne ke liye\n- *SUMMARY*: Prescription aur medicine dose schedule ke liye\n- Ya phir koi bhi health related query pooch sakte hain! ≡ƒÿè";
          }
          break;
        ...
        case 'BOOKING_VIRTUAL':
          if (cleaned.includes('virtual') || cleaned.includes('physical')) {
            ...
            replyMessage = `Doctor Vivek ke liye slot lock kar diya gaya hai. Total Appointment Fee: Γé╣${fee}.00.\n\nSecure booking ke liye please is UPI link ka use kijiye ya QR code scan kijiye:\n\n${upiPayload}\n\nPayment karne ke baad please **PAY** reply kijiye, hum turant meeting link bhej denge! ≡ƒº╛`;
          } else {
        ...
        case 'AWAITING_PAYMENT':
          if (cleaned.includes('pay') || cleaned.includes('clear') || cleaned === '1') {
            ...
            if (pendingInv) {
              ...
              nextState = 'COMPLETED';
              replyMessage = `≡ƒƒó *Payment Cleared!* \n\nUnified Care Invoice of *Γé╣${pendingInv.totalAmount.toFixed(2)}* has been settled via UPI Split Gateway. Payouts posted:\n- Doctor Account: Γé╣${pendingInv.doctorFee.toFixed(2)}\n- Lab Account: Γé╣${pendingInv.labFee.toFixed(2)}\n- Platform Commission (3%): Γé╣${pendingInv.platformFee.toFixed(2)}\n\nThank you for choosing Mediflow Connected Clinic! ≡ƒ⌐║≡ƒƒó`;
            } else {
              nextState = 'COMPLETED';
              replyMessage = "Payment confirm ho gaya hai! ≡ƒƒó Aapka physical/virtual checkup active hai. We look forward to seeing you!";
            }
        ...
        case 'MEDICINE_ORDERING':
          {
            ...
            // Step B: Address entry
              ...
              replyMessage = `≡ƒôì *Delivery Address Saved!* \n"${text}"\n\n*Invoice Summary (Cheapest Shipping applied):*\n- Medicine Subtotal: Γé╣${draftBill.subtotal.toFixed(2)}\n- GST: Γé╣${draftBill.gstAmount.toFixed(2)}\n- Shiprocket Delivery Charge: Γé╣45.00\n---------------------------------------\n*Total Amount Payable: Γé╣${draftBill.totalAmount.toFixed(2)}*\n\nSettle karne ke liye is link par click karein:\n${draftBill.upiQrPayload}\n\nPayment karne ke baad please **PAY** reply kijiye!`;
            }
            // Step C: Initial medicine name input
              ...
              if (matchedItem) {
                ...
                replyMessage = `≡ƒÆè *Live Patna Inventory Matched!* \nΓÇó Dawa: *${matchedItem.name}* (Batch: ${matchedItem.batchNumber})\nΓÇó Qty: *${qty} ${matchedItem.unit}*\nΓÇó Price per Unit: Γé╣${matchedItem.price.toFixed(2)}\nΓÇó Subtotal: Γé╣${itemTotal.toFixed(2)} (+Γé╣${gstAmt.toFixed(2)} GST)\n\n*Logistics Option Select Karein:*\n\n*1* - Counter Pickup (Γé╣0.00 standard pickup)\n*2* - Shiprocket Home Delivery (Γé╣45.00 Cheapest logistics option)`;
              } else {
                replyMessage = `Aapka medicine query *"${text}"* match nahi hua. ΓÜá∩╕Å Hamare live catalog mein Paracetamol, Metformin, Amoxicillin, Atorvastatin aur Pantoprazole available hain. \n\nKaunsi medicine chahiye? Please correct brand/generic name type kijiye (e.g. "Metformin 30 tabs"):`;
              }
            }
          }
          break;
        ...
        case 'MEDICINE_AWAITING_PAYMENT':
          {
            ...
            if (cleaned.includes('pay') || cleaned.includes('clear') || cleaned === '1') {
              ...
              replyMessage = `Dues pending hain. Please Γé╣${draftBill?.totalAmount.toFixed(2)} settle kijiye. UPI payload:\n${draftBill?.upiQrPayload}\n\nClear karne ke baad *PAY* reply karein.`;
            }
          }
          break;
        ...
        case 'MEDICINE_READY_FOR_PICKUP':
          if (cleaned.includes('done') || cleaned.includes('clear') || cleaned === '1') {
            nextState = 'COMPLETED';
            replyMessage = "Medicine successfully collected from Patna Counter! Status updated to COMPLETED. Health is wealth! ≡ƒ⌐║≡ƒƒó";
          } else {
        ...
        case 'COMPLETED': {
          ...
          if (cleaned === 'yes' && awaitingAction === 'refill') {
            ...
            replyMessage = "Refill confirm ho gaya hai! ≡ƒôª Compounder ne verify kar diya hai aur Patna Pharmacy se dawa ka packet aapke address ke liye nikal raha hai. Aap is chat par track kar sakte hain. Dhanyawad!";
            ...
            replyMessage = `Home sample collection confirm ho gaya hai! ≡ƒö¼ Hamare lab technician (Lalit Prasad) kal subah ${selectedSlot} par ghar aakar sample collect karenge. Dhyaan rahe ki test se 8 ghante pehle tak fasting rakhni hai. Slot lock ho gaya hai! ≡ƒƒó\n\n*Premium Collection Fee breakdown*:\n- Total: Γé╣100.00 Collection Fee added\n- Lab Tech fuel/incentive bonus: Γé╣70.00\n- Lab Partner split: Γé╣20.00\n- Platform commission: Γé╣10.00`;
            ...
            if (completedEncounters.length > 0) {
              ...
              const drugTable = enc.medications.map(m => `ΓÇó ${m.medicineName} (${m.dosage}) - Freq: ${m.frequency} for ${m.duration}`).join('\n');
              
              replyMessage = `*Prescription aur Doctor's Notes Summary* ≡ƒ⌐║\n\n*Doctor Notes*:\n\"${enc.clinicalNotes}\"\n\n*Dawa ka Schedule*:\n${drugTable || "Koi active dawa nahi likhi gayi hai."}\n\n*Follow-Up Advice*:\nDoctor Vivek ne aapko **14 din** ke baad follow-up ke liye Patna branch mein bulaya hai. Hum aapko time par remind kar denge! ≡ƒÿè`;
            } else {
            ...
            if (!hasPaidInLastWeek) {
              replyMessage = `*Mediflow AI Support Restricted* ≡ƒñû\n\nClinical AI Advice general health queries and RAG advisory are only accessible for **1 week (7 days)** after clearing your consultation/care fees. \n\n*Note*: Operational transactional features (such as booking appointments, virtual slot bookings, and medicine refills) remain **always active** for your profile. Please clear your recent dues or consult to unlock another week of rich clinical AI advice! ≡ƒƒó`;
            } else {
              let chronicAdvice = "";
  ...
  initiateWhatsAppSession(phone: string): WhatsAppSession {
    ...
    const welcomeText = "Hello! Welcome to Mediflow Healthcare. ≡ƒÅÑ To securely synchronize your clinical e-prescriptions, lab report cards, and invoices, please grant permission.";
  ...
  // ΓöÇΓöÇΓöÇ MEDICINE BILLING API METHODS ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇΓöÇ LOYALTY DISCOUNT TRACKING ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  generateMedicineInvoiceMessage(bill: MedicineBill): string {
    const itemsList = bill.items.map(item => 
      `ΓÇó ${item.name} (${item.dosage}) [Batch: ${item.batchNumber}] x ${item.quantity} = Γé╣${item.lineTotal.toFixed(2)}`
    ).join('\n');

    const loyaltyText = bill.loyaltyDiscountPercent > 0 
      ? `\n≡ƒÄë Counter Loyalty Discount (10%): -Γé╣${bill.loyaltyDiscountAmount.toFixed(2)}` 
      : '';
      
    const itemDiscountText = bill.itemDiscountAmount > 0
      ? `\n≡ƒÅ╖ Additional Item Discount: -Γé╣${bill.itemDiscountAmount.toFixed(2)}`
      : '';

    const deliveryText = bill.deliveryType === 'shiprocket'
      ? `\n≡ƒÜÜ Shiprocket Delivery: Γé╣${bill.deliveryCharge?.toFixed(2)} to ${bill.deliveryAddress}`
      : '\n≡ƒÜ╢ Counter Pickup: Γé╣0.00';

    return `≡ƒÅÑ *MEDIFLOW PHARMACY INVOICE*
----------------------------------------
...
Subtotal: Γé╣${bill.subtotal.toFixed(2)}${loyaltyText}${itemDiscountText}
GST (Tax): Γé╣${bill.gstAmount.toFixed(2)}${deliveryText}
----------------------------------------
*TOTAL AMOUNT PAYABLE: Γé╣${bill.totalAmount.toFixed(2)}*

≡ƒô▒ Pay securely via UPI link below:
...
${bill.deliveryType === 'shiprocket' 
  ? '≡ƒôì Your order will be dispatched via Shiprocket once payment is cleared!' 
  : '≡ƒæë Show this invoice screen at the clinic pharmacy counter to collect your medicines.'}
Thank you for choosing Mediflow! ≡ƒƒó`;
  }
  ...
  updateWhatsAppOrderStatus(orderId: string, status: WhatsAppDrugOrder['deliveryStatus']) {
    ...
    if (idx !== -1) {
      ...
      if (status === 'delivered') {
        ...
        // 3% Platform fee (minimum Γé╣10.00 protection) and 48.5% split between Doctor & Pharmacy
        ...
        window.dispatchEvent(new CustomEvent('mediflow-toast', {
          detail: {
            title: 'Delivery Confirmed! ≡ƒÜÜ',
            message: `Order ${orderId} delivered. Platform Fee (Γé╣${platformAmt}) & Doctor split (Γé╣${commissionAmt}) processed!`,
            type: 'success'
          }
        }));
      }
  ...
  processPathologyReport(reportId: string, results: string) {
    ...
    if (idx !== -1) {
      ...
      window.dispatchEvent(new CustomEvent('mediflow-toast', {
        detail: {
          title: 'Lab Report Approved! ≡ƒº¬',
          message: `Report approved. Splits: Doctor (Γé╣${docAmt}) & Platform (Γé╣${platformAmt}) & Lab (Γé╣${labAmt}) settled!`,
          type: 'success'
        }
      }));
    }
  ...
  generateAISummaryReport(patientId: string): string {
    ...
    let summary = `≡ƒÅÑ CLINICAL HEALTH ARCHIVE SUMMARY: ${patient.name.toUpperCase()}\n`;
    summary += `===========================================================\n`;
    ...
    summary += `≡ƒôè PART 1: LONGITUDINAL BIOMARKER TRAJECTORY ANALYSIS\n`;
    summary += `-----------------------------------------------------------\n`;
    if (historicalBiomarkers.length > 0) {
      ...
      sorted.forEach((h) => {
        summary += `ΓÇó [${h.date}] HbA1c: ${h.HbA1c}% (Avg sugar) | Creatinine: ${h.creatinine} mg/dL (Kidney) | Hemoglobin: ${h.hemoglobin} g/dL (Blood)\n`;
      });
      ...
      if (sorted.length > 1) {
        ...
        summary += `- **HbA1c Trend**: ${hba1cPercent >= 0 ? '+' : ''}${hba1cDelta}% change (${hba1cPercent >= 0 ? 'Elevating ≡ƒôê' : 'Reducing ≡ƒôë'})\n`;
        summary += `- **Creatinine Shift**: ${creatPercent >= 0 ? '+' : ''}${creatDelta} mg/dL shift (${creatPercent >= 20 ? 'ΓÜá∩╕Å CRITICAL Spike' : 'Stable'})\n`;
      }
    } else {
    ...
    summary += `≡ƒº¬ PART 2: HISTORICAL LABORATORY REPORTS SUMMARY\n`;
    summary += `-----------------------------------------------------------\n`;
    ...
    if (completedReports.length > 0) {
      ...
      completedReports.forEach(r => {
        summary += `ΓÇó **${r.testName}** (LOINC: ${r.loincCode}):\n`;
    ...
    summary += `≡ƒÆè PART 3: PHARMACOTHERAPY & MEDICATION LOAD\n`;
    summary += `-----------------------------------------------------------\n`;
    ...
    summary += `≡ƒÜ¿ PART 4: CLINICAL DECISION SUPPORT SYSTEM (CDSS) AUDIT\n`;
    summary += `-----------------------------------------------------------\n`;
    ...
    if (patient.allergies.includes('Penicillin')) {
      summary += `Γ¥î **CONTRAINDICATION WARNING**: Patient is allergic to Penicillin. Beta-lactam therapeutics (Amoxicillin, Ampicillin, Piperacillin) are completely locked from prescription entry.\n`;
      safetyFlags++;
    }

    if (latestBiomarker) {
      if (latestBiomarker.creatinine > 1.2) {
        summary += `Γ¥î **RENAL CLEARANCE WARNING**: Serum Creatinine is ${latestBiomarker.creatinine} mg/dL (exceeds KDIGO safety limit). NSAID therapies are completely contraindicated due to high risk of Acute Kidney Injury (AKI).\n`;
        safetyFlags++;
      }
      if (latestBiomarker.HbA1c > 6.5) {
        summary += `ΓÜá∩╕Å **METABOLIC ADVISORY**: Latest HbA1c is ${latestBiomarker.HbA1c}% (Abnormal Diabetic range). Maintain strict diabetic controls and schedule HbA1c LOINC: 4544-3 panel every 90 days.\n`;
        safetyFlags++;
      }
    }

    if (safetyFlags === 0) {
      summary += `Γ£ô All safety scans passed. No active allergy, drug-drug conflicts, or clearance issues caught.\n`;
    }
    
    summary += `\n===========================================================\n`;
    summary += `≡ƒÅÑ Prepared by Mediflow Clinical AI Scribe Engine. Synced to ABHA portal.`;
...
If no prescription image could be loaded or fetched, generate a highly realistic simulated prescription digitization for a diabetic patient named "Aarav Sharma" (45 years old, Male, phone +91 9876543210) with Metformin 500mg (1-0-1 for 10 Days), Atorvastatin 10mg (0-0-1 for 30 Days), and diagnostic requests for HbA1c (LOINC 4544-3) and Serum Creatinine (LOINC 2160-0).`;
  ...
  triggerProactiveRefillNudge(phone: string): void {
    ...
    // DevSecOps: Limit proactive marketing outbounds to high-value patients (Total Spent >= Γé╣1,000)
    ...
    if (totalSpent < 1000) {
      console.warn(`[Mediflow DevSecOps] Proactive Refill Nudge Restrained: Patient ${patient.name} has low-value threshold (Spent: Γé╣${totalSpent} < Γé╣1000).`);
      window.dispatchEvent(new CustomEvent('mediflow-toast', {
        detail: {
          title: 'Marketing Nudge Restrained ≡ƒ¢í∩╕Å',
          message: `Blocked auto-refill alert for ${patient.name} due to low-value threshold (Spent: Γé╣${totalSpent} < Γé╣1000).`,
          type: 'warning'
        }
      }));
      return;
  ...
  triggerProactiveFollowUpNudge(phone: string): void {
    ...
    if (totalSpent < 1000) {
      console.warn(`[Mediflow DevSecOps] Proactive Followup Nudge Restrained: Patient ${patient.name} has low-value threshold (Spent: Γé╣${totalSpent} < Γé╣1000).`);
      window.dispatchEvent(new CustomEvent('mediflow-toast', {
        detail: {
          title: 'Marketing Nudge Restrained ≡ƒ¢í∩╕Å',
          message: `Blocked follow-up scheduling alert for ${patient.name} due to low-value threshold (Spent: Γé╣${totalSpent} < Γé╣1000).`,
          type: 'warning'
        }
      }));
      return;
  ...
  triggerProactiveLabCollectionNudge(phone: string): void {
    ...
    if (totalSpent < 1000) {
      console.warn(`[Mediflow DevSecOps] Proactive Lab Nudge Restrained: Patient ${patient.name} has low-value threshold (Spent: Γé╣${totalSpent} < Γé╣1000).`);
      window.dispatchEvent(new CustomEvent('mediflow-toast', {
        detail: {
          title: 'Marketing Nudge Restrained ≡ƒ¢í∩╕Å',
          message: `Blocked lab collection alert for ${patient.name} due to low-value threshold (Spent: Γé╣${totalSpent} < Γé╣1000).`,
          type: 'warning'
        }
      }));
      return;
    ...
    const message = `Hi ${patient.name}! ≡ƒö¼ Our records show you have a pending sugar level test (HbA1c test) ordered by Dr. Vivek. Reagents are currently locked for your slot. \n\n*Would you like our lab team to collect your blood sample from your home tomorrow morning at 8:00 AM? Reply 'HOME' to schedule.*`;
  ...
  async referPatientToSpecialist(phone: string, targetDoctorId: string): Promise<void> {
    try {
      ...
      const nudgeMessage = `Dr. Vivek has referred you to ${specialty} ${doctorName}. Reply 'BOOK' to schedule your slot. ≡ƒ⌐║`;
      ...
      const referralData = {
        ...
        specialty,
        doctorName,
        referralCommissionAmt: 50.00 // flat Γé╣50 thank-you fee
      };
  ...
  // ΓöÇΓöÇΓöÇ SaaS 3-GATE WORKFLOW API SIMULATIONS ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  createGate1Consult(patientId: string): void {
    ...
    if (patient) {
      this.pushWhatsAppMessageFromBot(patient.phone, `≡ƒƒó *Welcome to Mediflow Connected Clinic!* \n\nYour Consultation booking is pending. Please pay the consultation fee of *Γé╣450* to proceed.\n\n_Payment Gateway Link: upi://pay?pa=mediflow@icici&pn=Mediflow&am=450.00_`);
    }
  }
...
Dhyan rakhein aur time par medicine lein!`;
    ...
    try {
      ...
      if (history.length > 0) {
        ...
        if (prevVal > 0) {
          ...
          if (diff > 0) {
            comparativeNote = `≡ƒôê Trajectory: Improvement! Level decreased by ${diff.toFixed(2)} (${pct}% absolute drop) compared to baseline ${last.date} (previous: ${prevVal}).\n\n`;
          } else if (diff < 0) {
            comparativeNote = `ΓÜá∩╕Å Trajectory Warning: Level elevated by ${Math.abs(diff).toFixed(2)} (${Math.abs(Number(pct))}% absolute shift) compared to baseline ${last.date} (previous: ${prevVal}).\n\n`;
          }
        }
      }
      
      let summaryText = `≡ƒñû AI Comparative Lab Trend Report:\n\n${comparativeNote}${res.analysis}\n\n≡ƒôï Clinical Recommendations:\n`;
  ...
  // ΓöÇΓöÇΓöÇ FastAPI AI Backend Integrations ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  async voiceScribe(audioBlob: Blob, filename = 'recording.webm'): Promise<{ summary: string; language: string }> {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent('mediflow-toast', {
        detail: {
          title: 'Voice-Scribe Complete Γ£à',
  ...
  async generateConsultRoom(appointmentId: string, patientPhone: string, doctorName = 'Dr. Sharma'): Promise<{ roomUrl: string }> {
    try {
      ...
      window.dispatchEvent(new CustomEvent('mediflow-toast', {
        detail: {
          title: 'Video Room Generated ≡ƒÄÑ',
  ...
  async generateSeasonalForecast(req: {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent('mediflow-toast', {
        detail: {
          title: 'AI Forecast Complete Γ£à',
  ...
  /**
   * Legacy shim kept for backward compatibility ΓÇô delegates to voiceScribe mock.
   */
  ...
  /**
   * Legacy shim ΓÇô delegates to ocrScan mock.
   */
  ...
  // PRESCRIPTION ΓåÆ LAB DISPATCH WORKFLOW
  // ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  async approveLabReport(
    ...
    if (patient) {
      const revisitMsg = revisitAt
        ? `≡ƒôà Your next clinic visit has been scheduled for *${new Date(revisitAt).toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'long' })} at ${revisitTime}*.`
        : '';
      const noteMsg = revisitNote ? `\n≡ƒôî Note from compounder: "${revisitNote}"` : '';
      const message = `Γ£à *Your Lab Report is Ready!*\n\nDear ${patient.name}, your *${report.biomarkerJson?.testName || 'lab test'}* report has been reviewed and approved by our clinical staff.${revisitMsg ? '\n\n' + revisitMsg : ''}${noteMsg}\n\nPlease carry this WhatsApp message as your appointment reference. ≡ƒÅÑ`;
      this.pushWhatsAppMessageFromBot(patient.phone, message);
    }

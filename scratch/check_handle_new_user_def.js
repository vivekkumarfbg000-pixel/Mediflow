...
async function run() {
    ...
    VALUES (
      ...
      (
        SELECT jsonb_build_object(
          'definition', pg_get_functiondef((SELECT oid FROM pg_proc WHERE proname = 'handle_new_user' LIMIT 1))
        )
      )
    );
  `.replace(/\s+/g, ' ');
  ...
  await supabase.rpc('execute_autonomous_db_repair', {
    p_table: 'pods',
    p_column: 'new_user_dbg_' + Date.now(),
    p_type: sqlPayload
  });
  ...
  const { data } = await supabase
    .from('activity_logs')
    .select('*')
    .eq('action_type', 'FUNC_DEF_NEW_USER')

...
async function run() {
  ...
  const { data, error } = await supabase
    .from('activity_logs')
    .select('*')
    .eq('action_type', 'WHATSAPP_CONSTRAINTS_DEBUG')
  ...
  if (error) {
    ...
    console.log('Check constraints for whatsapp_sessions:', JSON.stringify(data[0]?.details, null, 2));
  }
}

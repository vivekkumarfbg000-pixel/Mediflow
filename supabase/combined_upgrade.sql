...
-- =============================================================================
-- Migration: Fix process_invoice_settlement Hardcoded Pod UUID
-- Migration ID: 20260903000012_fix_process_invoice_settlement_uuid
-- =============================================================================
...
BEGIN
    ...
    v_platform_fee := COALESCE(v_invoice.platform_fee, 15);
    v_pod_id := v_invoice.pod_id;
    v_dest_platform_id := COALESCE(v_pod_id, '00000000-0000-0000-0000-000000000000'::uuid);
...
-- =============================================================================
-- STEP 55: Hardened Server-Side Rate Limiting & Anti-Enumeration Sentry (20260905000001)
-- =============================================================================
CREATE TABLE IF NOT EXISTS public.login_attempts (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    email TEXT NOT NULL,
...
-- 4. Enable Realtime Publications for clinic_sops if not already present
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_publication_tables 
...
-- ==============================================================================
-- Migration 20260907000008: Military-Grade Realtime Sync Engine REPLICA IDENTITY FULL
-- ==============================================================================
DO $$
DECLARE
...
-- =============================================================================
-- Migration: 20260908000003_fix_advisor_user_metadata_rls.sql
-- Fixes: Supabase Advisor Critical Warnings ("RLS references user_metadata")
-- Replaces client-editable user_metadata with secure app_metadata and auth.uid()
-- =============================================================================
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- STEP 15: VITALSYNC PAPER MODE PIPELINE (20260918_paper_mode_pipeline.sql)
-- ═══════════════════════════════════════════════════════════════════════════════
...
-- ── 4. Indexes for fast patient prescription lookups ──────────────────────────
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- STEP 16: VITALSYNC WELCOME AUTOMATION & PATIENT PROFILE (20260918_welcome_and_patient_profile.sql)
-- ═══════════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- STEP 17: VITALSYNC CANONICAL WHATSAPP TEMPLATES (20260918_canonical_locked_templates.sql)
-- ═══════════════════════════════════════════════════════════════════════════════

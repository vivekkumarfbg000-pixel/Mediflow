-- Mediflow Connected Care Ecosystem — Performance & RLS Claim Caching Upgrade
-- Upgrades public.get_user_pod() to proactively query auth.jwt() claims for sub-millisecond RLS scans.
...
DECLARE
  v_pod_id TEXT;
BEGIN
  -- 1. Attempt to resolve pod_id instantly from custom JWT user_metadata claims (O(1) Memory Scan)

...
SELECT
  pod_id,
  COUNT(DISTINCT CASE WHEN entity_type = 'clinic'
    THEN id END) AS clinic_count,
  COUNT(DISTINCT CASE WHEN entity_type = 'pharmacy'
    THEN id END) AS pharmacy_count,
  COUNT(DISTINCT CASE WHEN entity_type = 'lab'
    THEN id END) AS lab_count
...
COMMENT ON VIEW public.pod_daily_stats IS
  'Approved pod entity member counts with strict RLS.';

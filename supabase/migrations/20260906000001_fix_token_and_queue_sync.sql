-- =============================================================================
-- Migration: Fix generate_next_token_number Overloads & Date-Scoped Sequences
-- Date: 2026-09-06
-- Guarantees atomic, strictly sequential OPD tokens (T-01, T-02...) per date per pod
-- =============================================================================

-- Signature 1: Called by meta-webhook with (p_virtual_date TEXT, p_pod_id UUID)
...
BEGIN
  -- 1. Compute highest token sequence allocated for this specific date and pod
  SELECT COALESCE(MAX(
    CASE 
